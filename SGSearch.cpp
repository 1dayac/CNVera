...
//
void SGWalkBuilder::finishCurrentWalk(bool add)
{
    if (add)
...
void SGSearch::findAllWalks(Vertex* pX, EdgeDir initialDir,
  int maxDistance, size_t maxNodes,
  SGWalkVector& outWalks)
{
  SGSearchTree searchTree(pX, NULL, initialDir, maxDistance, 1000000);
  ...
  if (!searchTree.wasSearchAborted())
  {
    // Extract the walks from the graph as a vector of edges
    SGWalkBuilder builder(outWalks, false);
    searchTree.buildWalksToAllLeavesWithD(builder, maxDistance);
  }
...
void SGSearch::extendWalk(SGWalk& walk, SGWalkVector& walkVector)
{
  Vertex* lastVertex = walk.getLastVertex();
  Edge* lastEdge = walk.getLastEdge();
  EdgeDir dir = lastEdge->getDir();

  if (walk.getStartToEndDistance() > 100000)
    return;
  for (auto d : lastVertex->getEdges())
  {
    if (!((lastEdge->getTwin()->getMatchCoord().interval.start == d->getMatchCoord().interval.start && lastEdge->getTwin()->getMatchCoord().interval.start == 0) || (d->getMatchCoord().interval.start != 0 && lastEdge->getTwin()->getMatchCoord().interval.start != 0)))
    {
      if (d->getEnd()->getSeqLen() > 1000 && d->getEnd()->getEstimatedCN() == 1)
        continue;
      SGWalk newWalk = walk;
      newWalk.addEdge(d);
      walkVector.push_back(newWalk);
    }
  }
}