...
# Parameters
SGA_BIN=/home/meleshkod/sga/src/SGA/sga
...
#
# Scaffolding/Paired end resolution - without refining
# 
CTGS=assemble.m$OL-contigs.fa
GRAPH=assemble.m$OL-graph.asqg.gz

# Realign reads to the contigs
~/sga/src/bin/sga-align --name ecoli.old.pe $CTGS $IN1 $IN2

# Make contig-contig distance estimates
~/sga/src/bin/sga-bam2de.pl -n $MIN_PAIRS --prefix libPE ecoli.old.pe.bam

# Make contig copy number estimates
~/sga/src/bin/sga-astat.py -m $MIN_LENGTH ecoli.old.pe.refsort.bam > libPE.astat
...
python ~/sga/src/examples/countEntriesContigRef.py $CTGS DH10B-K12.fasta libPE.astat



~/ASQGParser/ASQGParser $GRAPH unique.txt results.txt



mv $GRAPH.out assemble.m$OL-graph-refined.asqg

#
# Scaffolding/Paired end resolution - without refining
# 
GRAPH2=assemble.m$OL-graph-refined.asqg

# Realign reads to the contigs
~/sga/src/bin/sga-align --name ecoli.new.pe $CTGS $IN1 $IN2

# Make contig-contig distance estimates
~/sga/src/bin/sga-bam2de.pl -n $MIN_PAIRS --prefix libPE2 ecoli.new.pe.bam

# Make contig copy number estimates
~/sga/src/bin/sga-astat.py -m $MIN_LENGTH ecoli.new.pe.refsort.bam > libPE2.astat

$SGA_BIN scaffold -m $MIN_LENGTH --pe libPE.de -a libPE2.astat -o scaffolds.n$MIN_PAIRS.scaf $CTGS
$SGA_BIN scaffold2fasta -m $MIN_LENGTH -a $GRAPH2 -o scaffolds.n$MIN_PAIRS.fa -d $SCAFFOLD_TOLERANCE --use-overlap --write-unplaced scaffolds.n$MIN_PAIRS.scaf
