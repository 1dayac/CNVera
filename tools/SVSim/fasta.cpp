...
void FastaWriter::writeSequences(std::vector<FastaRecord>& record)
{
  for (int i = 0; i < record.size(); ++i)
  {
    outputStream_ << ">" << record[i].id_ << std::endl;
    for (int j = 0; j < record[i].sequence_.length(); ++j)
    {
      outputStream_ << record[i].sequence_[j];
      if (j % 80 == 0 && j != 0)
      {
        outputStream_ << std::endl;
      }
    }
    if(record[i].sequence_.length() - 1 % 80 != 0)
    {
      outputStream_ << std::endl;
    }
  }
