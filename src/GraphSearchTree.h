...
class GraphSearchTree
{
    ...
    public:
        ...
        template<typename BUILDER>
        void buildWalksToAllLeavesWithD(BUILDER& walkBuilder, int distance);
    ...
    private:
        ...
        template<typename BUILDER>
        void _buildWalksToLeaves(const _SearchNodePtrDeque& queue, BUILDER& walkBuilder, int distance);
...
// Construct walks representing every path from the start node with distance
template<typename VERTEX, typename EDGE, typename DISTANCE>
template<typename BUILDER>
void GraphSearchTree<VERTEX, EDGE, DISTANCE>::buildWalksToAllLeavesWithD(BUILDER& walkBuilder, int distance)
{
  // Construct a queue with all leaf nodes in it
  _SearchNodePtrDeque completeLeafNodes;
  _makeFullLeafQueue(completeLeafNodes);

  _buildWalksToLeaves(completeLeafNodes, walkBuilder, distance);
}
...
template<typename VERTEX, typename EDGE, typename DISTANCE>
template<typename BUILDER>
void GraphSearchTree<VERTEX, EDGE, DISTANCE>::_buildWalksToLeaves(const _SearchNodePtrDeque& queue, BUILDER& walkBuilder, int distance)
{
  for (typename _SearchNodePtrDeque::const_iterator iter = queue.begin();
    iter != queue.end();
    ++iter)
  {
    ...
    walkBuilder.finishCurrentWalk(true);
  }
}
